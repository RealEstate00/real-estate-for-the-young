...
class LHAnnouncementCrawler(BaseCrawler):
    """LH 공고 크롤러"""
    
    def __init__(self, progress: Progress):
        super().__init__(progress)
        self.list_url = "https://www.lh.or.kr/portal/board/list.do?boardId=notice"
        self.platform_code = "lh_ann"
    
    def crawl(self):
        """LH 공고 크롤링 실행"""
        self.progress.update("[START] LH Announcement Crawler")
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                # LH 공고 필터링 적용 (서울 관련, 2024년 이후)
                filtered_details = []
                for title, detail_desc in details:
                    if filter_lh_announcement_title(title):
                        filtered_details.append((title, detail_desc))
                
                self.progress.update(f"[INFO] Found {len(details)} total items, {len(filtered_details)} after LH filtering")
