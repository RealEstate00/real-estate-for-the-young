...
from backend.services.data_collection.housing.normalized.normalizer import DataNormalizer
from backend.services.data_collection.housing.normalized.data_quality_enhancer import DataQualityEnhancer, print_quality_report
from backend.services.loading.housing.housing_db_loader import HousingLoader
from backend.services.db.common.db_utils import get_engine
from backend.libs.utils.paths import make_normalized_dir
...
def get_normalized_output_path(raw_file: Path) -> Path:
    ...
    # 예: data/raw/housing/sohouse/2025-09-15/raw.csv
    ...
    # make_normalized_dir 함수 사용
    return make_normalized_dir(platform_name)
...
def clean_normalized_data(platform: str = None, date: str = None) -> None:
    """기존 정규화된 데이터 삭제"""
    from backend.libs.utils.paths import HOUSING_NORMALIZED_DIR
    normalized_dir = HOUSING_NORMALIZED_DIR
